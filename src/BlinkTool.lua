...
function BlinkToolLoader()
    ...
    function BlinkTool:_SpawnBlinkParticles()
        ...
        for i = 0, 8 do
            SpawnParticle(VecAdd(cpos,
                VecScale(VecCross(dir, Vectors.DirRandom()), 2)),
                VecAdd(velocity, Vectors.DirRandom()),
    ...
    function BlinkTool:Tick(dt)
        -- Handle input
        if (self.KEYBIND or (not InputPressed("lmb") and self:_InHand())) and GetBool("game.player.canusetool") then
            if (
                not self.KEYBIND and self:_InHand() and InputPressed("rmb") or
                    self.KEYBIND and InputPressed(self.KEYBIND))
            ...
            elseif (
                not self.KEYBIND and self:_InHand() and InputReleased("rmb") or
                    self.KEYBIND and InputReleased(self.KEYBIND)
                ) and
